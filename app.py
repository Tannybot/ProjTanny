...
def add_bg_from_local(image_file):
    try:
        with open(image_file, "rb") as f:
            ...
            h1, h2, h3, h4, h5, h6, .css-10trblm, .css-1544g2n {{
                color: #FFFFFF !important;
                font-weight: 600;
            }}
            .stButton button {{
        ...
        logging.warning(f"Background image {image_file} not found. Using fallback style.")
        return """
        <style>
        .stApp {
            background-color: #000033;
...
# Streamlit UI setup
st.set_page_config(page_title="Government Event Scheduler", layout="centered")

# Custom title with styling
st.markdown('<div class="main-title"><h1>Welcome to the Government Event Scheduler</h1></div>', unsafe_allow_html=True)

# Try to add background image, with error handling
try:
...
# Navigation
menu = ["Create Event", "View Events", "Register for Event", "Event Dashboard", "Analytics"]
            ...
            with st.expander(f"{event['name']} - {event['date']}"):
                ...
                with col1:
                    if event['registered'] < event['capacity']:
                        if st.button(f"Quick Register", key=f"quick_reg_{event_id}", use_container_width=True):
                            st.session_state['selected_event'] = event_id
                            st.session_state['menu_choice'] = "Register for Event"
                            safe_rerun()
                    else:
    ...
    with right_col:
        for event_id, event in event_items[mid_point:]:
            with st.expander(f"{event['name']} - {event['date']}"):
                ...
                with col1:
                    if event['registered'] < event['capacity']:
                        if st.button(f"Quick Register", key=f"quick_reg_r_{event_id}", use_container_width=True):
                            st.session_state['selected_event'] = event_id
                            st.session_state['menu_choice'] = "Register for Event"
                            safe_rerun()
                    else:
...
# Dashboard
    ...
    if not events:
        ...
        if registrations:
            ...
            st.dataframe(reg_details[['name', 'email', 'event_id', 'timestamp']])
        else:
